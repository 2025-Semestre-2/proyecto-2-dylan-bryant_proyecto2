...
WHERE o.CustomerID IN (SELECT CustomerID FROM Limon.Sales.Customers)
  AND o.SalespersonPersonID IN (SELECT PersonID FROM Limon.Application.People)
  AND o.PickedByPersonID IN (SELECT PersonID FROM Limon.Application.People)
  AND o.ContactPersonID IN (SELECT PersonID FROM Limon.Application.People)
  AND o.BackorderOrderID IS NULL
  AND o.OrderID NOT IN (SELECT OrderID FROM Limon.Sales.Orders) -- <-- evita duplicados
ORDER BY NEWID();

SELECT COUNT(*)
FROM WideWorldImportersP.Sales.Orders o
WHERE o.BackorderOrderID IS NOT NULL
  AND o.BackorderOrderID IN (SELECT OrderID FROM Limon.Sales.Orders);
...
WHERE ol.OrderID IN (SELECT OrderID FROM Limon.Sales.Orders)
...
WHERE i.CustomerID IN (SELECT CustomerID FROM Limon.Sales.Customers)
  AND i.OrderID IN (SELECT OrderID FROM Limon.Sales.Orders)
...
insert into Sales.InvoiceLines(
 ...
 il.InvoiceLineID
  ...
  where il.InvoiceID in (select InvoiceID from Limon.Sales.Invoices)
  and il.StockItemID in (select StockItemID from Limon.Warehouse.StockItems)
...
sit.StockItemTransactionID
  ...
  where sit.StockItemID in ( select StockItemID from Limon.Warehouse.StockItems)
  and sit.CustomerID in ( select CustomerID from Limon.Sales.Customers)
  and sit.InvoiceID in ( select InvoiceID from Limon.Sales.Invoices)
  and (sit.PurchaseOrderID IS NULL OR sit.PurchaseOrderID IN (SELECT PurchaseOrderID FROM Limon.Purchasing.PurchaseOrders))
  and (sit.SupplierID IS NULL OR sit.SupplierID IN (SELECT SupplierID FROM Limon.Purchasing.Suppliers))
...
(
    'daddyYankee',
    HASHBYTES('SHA2_256', 'echepalante'),
    'Luis Figo',
    1,
    'FigoLuigi@gmail.com',
    GETDATE()
